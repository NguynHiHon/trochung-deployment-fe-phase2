...
const RoomDetailPage = () => {
  ...
  const submitComment = async () => {
    ...
    try {
      ...
      setNewComment('');
      // refresh stats (nếu view-all đang mở)
    ...
    <Box sx={{ maxWidth: 1200, mx: 'auto', p: 3 }}>
      ...
      <Grid container spacing={3}>
        ...
        <Grid item xs={12} md={12}>
          {/* Gallery hình ảnh mới (giống style ảnh mẫu) */}
          <Paper sx={{ overflow: 'hidden', borderRadius: 2, mb: 3, p: { xs: 0.5, md: 1 } }}>
            {(() => {
              ...
              return (
                <>
                  <Box sx={{ position: 'relative', height: { xs: 300, md: 420 }, borderRadius: 2, overflow: 'hidden', cursor: 'zoom-in' }} onClick={() => setImageViewerOpen(true)}>
                  <Box 
                    component="img" 
                      src={images[current]}
                    ...
                    {total > 1 && (
                      <>
                        <IconButton onClick={goPrev} sx={{ position: 'absolute', top: '50%', left: 8, transform: 'translateY(-50%)', bgcolor: 'rgba(0,0,0,0.5)', color: 'white', '&:hover': { bgcolor: 'rgba(0,0,0,0.7)' } }}>
                          <NavigateBefore />
                        </IconButton>
                        <IconButton onClick={goNext} sx={{ position: 'absolute', top: '50%', right: 8, transform: 'translateY(-50%)', bgcolor: 'rgba(0,0,0,0.5)', color: 'white', '&:hover': { bgcolor: 'rgba(0,0,0,0.7)' } }}>
                          <NavigateNext />
                        </IconButton>
                        <Box sx={{ position: 'absolute', right: 12, bottom: 12, bgcolor: 'rgba(0,0,0,0.55)', color: 'white', px: 1, py: 0.3, borderRadius: 1, fontSize: 12, fontWeight: 600 }}>
                          {current + 1}/{total}
                </Box>
                      </>
                        )}
                  ...
                  {total > 1 && (
                    <Box sx={{ display: 'flex', gap: 1, mt: 1, overflowX: 'auto', pb: 0.5 }}>
                      {images.map((img, idx) => (
                        <Box key={idx} onClick={() => setCurrentImageIndex(idx)} sx={{ cursor: 'pointer', borderRadius: 1, overflow: 'hidden', flex: '0 0 auto', border: idx === current ? '2px solid #1976d2' : '1px solid #e0e0e0' }}>
                          <Box component="img" src={img} alt={`${room.title} thumb ${idx + 1}`} sx={{ width: 120, height: 90, objectFit: 'cover', display: 'block' }} />
                        </Box>
                      ))}
                    </Box>
                  )}
          ...
          {imageViewerOpen && (() => {
            ...
            const total = images.length;
            const current = Math.min(currentImageIndex, Math.max(0, total - 1));
            const goPrev = (e) => { e.stopPropagation(); setCurrentImageIndex((i) => (i - 1 + total) % total); };
            const goNext = (e) => { e.stopPropagation(); setCurrentImageIndex((i) => (i + 1) % total); };
            const close = () => setImageViewerOpen(false);
            return (
              <Box onClick={close} sx={{ position: 'fixed', inset: 0, bgcolor: 'rgba(0,0,0,0.9)', zIndex: 2000, display: 'flex', alignItems: 'center', justifyContent: 'center', p: 2 }}>
                <IconButton onClick={(e) => { e.stopPropagation(); close(); }} sx={{ position: 'absolute', top: 12, right: 12, bgcolor: 'rgba(255,255,255,0.9)' }}>
                  <Close />
                </IconButton>
                {total > 1 && (
                  <IconButton onClick={goPrev} sx={{ position: 'absolute', left: 16, bgcolor: 'rgba(255,255,255,0.2)', color: 'white' }}>
                    <NavigateBefore />
                  </IconButton>
                )}
                <Box component="img" src={images[current]} alt={`${room.title} full ${current + 1}`} sx={{ maxWidth: '92vw', maxHeight: '92vh', objectFit: 'contain', borderRadius: 1 }} />
                {total > 1 && (
                  <IconButton onClick={goNext} sx={{ position: 'absolute', right: 16, bgcolor: 'rgba(255,255,255,0.2)', color: 'white' }}>
                    <NavigateNext />
                  </IconButton>
                )}
              </Box>
                      ...
                      {replyFor === c._id && (
                        <Stack direction="row" spacing={1} sx={{ mt: 1, pl: 5 }}>
                          <input
                            ...
                            onChange={(e) => { if (editFor?.id === `reply-${c._id}`) setEditText(e.target.value); else { setEditFor({ id: `reply-${c._id}` }); setEditText(e.target.value); } }}
                            onKeyDown={async (ev) => {
                              if (ev.key === 'Enter') {
                                const val = (editFor?.id === `reply-${c._id}` ? editText : ev.currentTarget.value).trim();
                                if (!val) return;
                                if (!accessToken) { showToast('Vui lòng đăng nhập để trả lời', 'warning'); return; }
                                try {
                                  const postId = room?.postId || room?.post || room?.id;
          ...
          <Paper sx={{ p: 3, mb: 3, borderRadius: 2 }}>
            ...
            <Grid container spacing={1.5} alignItems="stretch" sx={{ flexWrap: { xs: 'wrap', md: 'nowrap' } }}>
              {similarRooms.map((similarRoom) => (
                <Grid item xs={12} sm={4} md={4} key={similarRoom.id} sx={{ display: 'flex', flex: { md: '1 0 0' }, maxWidth: { md: '33.3333%' }, minWidth: 0 }}>
                  <Card variant="outlined" sx={{ 
                    ...
                    <Box sx={{ position: 'relative', height: 140, display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }}>
                      <Box 
                        component="img" 
                      ...
                      <IconButton 
                        ...
                        onClick={async () => {
                          ...
                          if (newFavorites.has(similarRoom.id)) {
                            ...
                            try { await FavoriteApi.removeFavorite(similarRoom.id); } catch (_) {}
                          } else {
                            newFavorites.add(similarRoom.id);
                            try { await FavoriteApi.addFavorite(similarRoom.id); } catch (_) {}
                          }
                          setFavorites(newFavorites);
                          localStorage.setItem('favoriteRoomIds', JSON.stringify([...newFavorites]));
